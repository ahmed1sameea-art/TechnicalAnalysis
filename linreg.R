...
# Indicator: Linear Regression (linreg)
# Student: Sameea Ahmed
# File: linreg.R
#
# Simple linear regression y = beta0 + beta1 * x fit over a sliding window
# (regressionLength points, offset back from the end of the series by
# regressionOffset). No statistical tests or error terms are computed -
# only slope, intercept, and the fitted (predicted) values, per the
# assignment's pseudocode. Implemented using only R's standard core
# functions (no libraries, no lm()).
# =============================================================================
...
linreg <- function(regressionSource, regressionLength, regressionOffset) {
  ...
  # Check if regressionLength is greater than the number of elements in
  # regressionSource
